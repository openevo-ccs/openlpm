...
import { ArrowLeft, ArrowRight, Clock, Compass, FileText, GitBranch, Grid3x3, ShieldAlert } from 'lucide-react'
...
export default function BranchLayout() {
  ...
  const [promotedToSlug, setPromotedToSlug] = useState<string | null>(null)
  ...
  // A promoted branch's real content has actually moved to its own project
  // (see migration 015's Thuringia move) -- without this, every tab here
  // (Explore/Schema/Coherence/Review) would just look empty, reading as
  // data loss rather than "this moved elsewhere."
  useEffect(() => {
    setPromotedToSlug(null)
    if (!branch?.promoted_to_project_id) return
    supabase
      .from('projects')
      .select('slug')
      .eq('id', branch.promoted_to_project_id)
      .maybeSingle()
      .then(({ data }) => setPromotedToSlug(data?.slug ?? null))
  }, [supabase, branch?.promoted_to_project_id])
  ...
  const nav = [
    { href: `/dashboard/${slug}/branches/${branch.slug}/explore`, content: <><Compass size={14} />Explore</>, end: false },
    ...
    { href: `/dashboard/${slug}/branches/${branch.slug}/coherence`, content: <><Grid3x3 size={14} />Coherence</> },
    { href: `/dashboard/${slug}/branches/${branch.slug}/review`, content: <><Clock size={14} />Review</> },
  ]
  ...
  return (
    <div>
      ...
      <p className="muted" style={{ marginBottom: 4 }}>
        {project.name}
        {parentLabel && <> — started from <b style={{ color: 'var(--text-primary)' }}>{parentLabel}</b></>}
      </p>
      {branch.description && <p style={{ marginBottom: 16 }}>{branch.description}</p>}

      {branch.status === 'promoted' ? (
        <div className="notice notice-ok" style={{ alignItems: 'center' }}>
          <span>This grew into its own project — everything that used to be here now lives there.</span>
          {promotedToSlug && (
            <Link to={`/dashboard/${promotedToSlug}`} className="btn btn-mini" style={{ marginLeft: 'auto' }}>
